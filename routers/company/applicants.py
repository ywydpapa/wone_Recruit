...
async def applicants_list(request: Request, job_id: int, q: str = "", sort: str = "newest"):
    ...
    try:
        ...
        where = ["c.job_id=?", "(c.source='direct' OR c.match_stage='submitted')"]
        params = [job_id]
        if q and q.strip():
        ...
        applicants = conn.execute(
            f"""SELECT c.id, c.status, c.source, c.created_at,
...
async def applicant_detail(request: Request, job_id: int, candidacy_id: int):
    ...
    try:
        ...
        if job is None:
            raise HTTPException(status_code=404, detail="공고를 찾을 수 없습니다.")
        cand = conn.execute(
            """SELECT c.*, u.name AS seeker_name, u.phone AS seeker_phone, u.email AS seeker_email
               FROM candidacies c
               JOIN users u ON c.seeker_user_id=u.id
        ...
        educations = careers = certifications = languages = []
        if resume:
            rid = resume["id"]
            educations = conn.execute(
                "SELECT * FROM resume_educations WHERE resume_id=? ORDER BY sort_order", (rid,)
            ).fetchall()
            careers = conn.execute(
                "SELECT * FROM resume_careers WHERE resume_id=? ORDER BY sort_order", (rid,)
            ).fetchall()
            certifications = conn.execute(
                "SELECT * FROM resume_certifications WHERE resume_id=? ORDER BY sort_order", (rid,)
            ).fetchall()
            languages = conn.execute(
                "SELECT * FROM resume_languages WHERE resume_id=? ORDER BY sort_order", (rid,)
            ).fetchall()
    finally:
        conn.close()
    return templates.TemplateResponse(
...
async def applicant_status_change(
    ...
    try:
        ...
        if job:
            apply_transition(conn, candidacy_id, status, user["id"], comment)
    finally:
        conn.close()
